...
openlog(array_key_exists('SHELL', $_SERVER) ? '' : "[client {$_SERVER['REMOTE_ADDR']}]", LOG_PID | LOG_NDELAY, LOG_USER);
...
class ErrorHandler
{
	 ...
	 * Set to FALSE to exit without a message. 
	 * 
	 * @var string
	 */
	public $errorPage;
	...
	/**
	 * Message how to contact the admin.
	 * Only shown when email address is supplied.
	 * 
	 * Tags from ErrorHandler::i()->messageVars will parsed into message.
	 *  
	 * @var string
	 */
	public $contactMessage = 'You may contact {$admin} by sending an e-mail to <a href="mailto:{$email}>{$email}</a>.';
	
	/**
	 * Set the HTTP response when a fatal error occurs.
	 * @var int
	 */
	public $httpResponse;
	...
	/**
	 * Alternative for error_get_last()
	 * 
	 * @return array
	 */
	static public function error_get_last()
	{
		if (!isset(self::$instance) || !(self::i()->isStarted() & 1)) return error_get_last();
	...
	{
		if (!($this->started & 1)) set_error_handler(array($this, "onError"));
	 ...
	 */	
	public function exitOnError($logged=false)
	{
		if (isset($this->httpResponse)) HTTP::response($this->httpResponse);
	    
		if ($this->errorPage === false) {
			exit(1);
		} elseif (empty($this->errorPage) || $_SERVER['PHP_SELF'] == $this->errorPage) {
			echo $this->getExitMessage($logged);
			exit(1);
		} else {
			HTTP::redirect($this->errorPage . (strstr($this->errorPage, '?') ? '&' : '?') . "logged=$logged");
			
			// if headers are already send
			echo $this->getExitMessage($logged);
			exit(1);
		}
	}
	...
	/**
	 * Get the message to display before exiting on an eeror
	 *
	 * @param   boolean    $logged      The error has succesfully been logged
	 */	
	public function getExitMessage($logged=false)
	{
		$msg = $this->parseMessage($this->errorMessage . "\n" . (!empty($this->email) ? $this->contactMessage . "\n" : ""));
