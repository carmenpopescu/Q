...
class HTTP
{
	 ...
	 * @return boolean 
	 */
	function redirect($url, $exit=true, $output=true)
	{
	...
	static public function headersDownload($filename, $filesize=null, $modified=null)
	{
		...
		if ($modified !== false && (isset($modified) || file_exists($filename))) {
			if (!isset($modified) || is_string($modified)) $modified = gmdate('D, d M Y H:i:s', isset($modified) ? $modified : filemtime($filename)) . 'GMT';
	 ...
	 * Be careful with using this in a production enviroment, since users can get stuck with an old version of the page.  
	 * 
