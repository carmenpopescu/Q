...
abstract class DB
{
	...
	const FETCH_ASSOC = 2;
	const FETCH_FULLARRAY = 3;
	const FETCH_PERTABLE = 4;
	const FETCH_VALUE = 5;
	const FETCH_RECORD = 6;
	const FETCH_ROLES = 7;
	...
	/**
	 * Return the fieldnames of the primairy key.
	 * @deprecated 
	 *
	 * @param string  $table
	 ...
	 * @return array
	 */
	public function getPrimaryKeys($table, $autoIncrementOnly=false, $asIdentifier=false)
	{
	    return (array)$this->getPrimaryKey($table, $autoIncrementOnly, $asIdentifier);
	}
	 ...
	 */
	public static function applyTableDefaults(&$properties)
	{
	 ...
	 * @return string
	 */
	abstract public function quote($value, $empty=null);
	
	/**
	 * Quotes a string so it can be safely used as a db or schema name.
	 ...
	 * @return string
	 */
	abstract public function quoteSchema($identifier);

	/**
	 * Quotes a string so it can be safely used as a table name.
	 * Dots are seen as seperator and are kept out of quotes.
	 * 
	 * Return NULL if $identifier is not valid.
	 *
	 ...
	 * @return string
	 */
	abstract public function quoteTable($identifier);

	/**
	 * Quotes a string so it can be safely used as a field name.
	 * Dots are seen as seperator and are kept out of quotes.
	 * 
	 * Return NULL if $identifier is not valid.
	 *
	 ...
	 * @return string
	 */
	abstract public function quoteField($identifier);
		
	/**
	 * Create a named argument which can be parsed
	 *
	 * @param string $name
	 * @param mixed $value
	 * @return object
	...
	{
	    return (object)array('#arg'=>true, 'name'=>$name, 'value'=>$value);
	}
	 ...
	 * 
	 * @param mixed $statement  String or query object
	 * @param Additional arguments are parsed into the query on placeholders
	 * @return DB_Result
	 */
	 ...
	 * @return DB_Result
	 */
	public function select($table, $fields=null, $criteria=null, $where=null)
	{
	    return $this->prepareSelect($table, $fields, $criteria, $where)->execute();
	}
	 ...
	 * 
	 * Returns the last inserted/updated id
	 * 
	 * @param string $table
	 * @param array  $values  Assasioted array as (fielname=>value, ...) or ordered array (value, ...) with 1 value for each field
